...
class Solution:
    def calPoints(self, operations: List[str]) -> int:
        stack = [] #to keep track of the scores
        for op in operations: 
            if op == "+": #if the operation is "+", we need to add the sum of the previous two scores to the stack
                stack.append(stack[-1] + stack[-2]) #we can access the previous two scores using stack[-1] and stack[-2]
            elif op == "D": #if the operation is "D", we need to add the double of the previous score to the stack
                stack.append(2 * stack[-1]) #we can access the previous score using stack[-1]
            elif op == "C": #if the operation is "C", we need to remove the previous score from the stack
                stack.pop() #we can remove the previous score using stack.pop()
            else:
                stack.append(int(op)) #if the operation is an integer, we need to add it to the stack
        return sum(stack) #we can return the sum of the scores in the stack using sum(stack)
#time complexity: O(n)
#space complexity: O(n)

#stack 2 (without using built-in sum function)
class Solution:
    def calPoints(self, operations: List[str]) -> int:
        stack, res = [], 0
        for op in operations:
