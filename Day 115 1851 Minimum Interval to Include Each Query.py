...
class Solution:
    def minInterval(self, intervals: List[List[int]], queries: List[int]) -> List[int]:
        ...
        for q in queries:
            ...
            res.append(cur)
        return res
#time complexity: O(mn), where m is the number of queries and n is the number of intervals in the input list, due to the nested loops.
#space complexity: O(m), where m is the number of queries in the input list, due to the space used for the output list, O(1) if we don't consider the space used for the output list.