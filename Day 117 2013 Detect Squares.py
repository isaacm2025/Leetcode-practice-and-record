...
class CountSquares:
    ...
    def count(self, point: List[int]) -> int:
        ...
        px, py = point
        for x, y in self.pts:
            if (abs(py - y) != abs(px - x)) or (px == x and py == y):
                continue
            res += self.ptsCount[(x, py)] * self.ptsCount[(px, y)]
        return res
#time complexity: O(n) for count where n is the number of points added
#space complexity: O(n) for storing the points and their counts